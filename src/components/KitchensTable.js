...
import UpdateKitchenModal from './UpdateKitchenModal';
import QRCode from 'react-qr-code';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';

function KitchensTable({ kitchens }) {
  const toast = useToast();
  const { fetchKitchens, deleteKitchen } = useKitchenContext();
  const [loading, setLoading] = useState(false);
  const qrRefs = useRef({});

  const handleDownloadPDF = async (kitchenId) => {
    const qrCanvas = qrRefs.current[kitchenId]?.current;
    ...
    setTimeout(async () => {
      try {
        ...
        pdf.save(`${kitchenId}_qr_code.pdf`);
      } catch (error) {
    ...
    <SimpleGrid bg='white' p={5} shadow='lg' borderRadius='lg' overflowX='auto'>
      {loading ? (
        ...
        <Table variant='simple'>
          ...
          <Tbody>
            {kitchens ? kitchens.map((kitchen, index) => {
              const { image, kitchenId, id, description } = kitchen;
              if (!qrRefs.current[kitchenId]) {
                qrRefs.current[kitchenId] = React.createRef();
              }
              return (
                <Tr key={index}>
                  <Td>
                  ...
                  <Td>
                    <Menu>
                      ...
                      <MenuList>
                        ...
                        <MenuItem>
                          <UpdateKitchenModal id={id} />
                        </MenuItem>
                        <MenuItem onClick={() => handleDelete(id)}>
                          Delete
                        </MenuItem>
                        <MenuItem onClick={() => handleDownloadPDF(kitchenId)}>
                          Download QR
                        </MenuItem>
                      </MenuList>
                    </Menu>
