...
function SingleTruckAssignInfo({ truckAssign }) {
  const { admins } = useAdminContext();
  const { orders } = useOrderContext();
  const { single_truckAssign_loading: loading } = useTruckAssignContext();
  ...
  useEffect(() => {
    console.log(truckAssign, "{PPPPPPPPPPPPPPPPP}")
    ...
    // creating new array having this truckAssign as the only orderItem
    // const truckAssignOrders = orders.reduce((arr, order) => {
    //   const item = order.orderItems.find((x) => x.truckAssign === id);
    //   if (item) {
    //     arr.push(item);
    //   }
    //   return arr;
    ...
    // const total = truckAssignOrders.reduce((total, order) => {
...
export default SingleTruckAssignInfo;
